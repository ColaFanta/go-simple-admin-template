	...
	return Do(func() Unit {
		...
		q = q.Joins(
			...
			func(db gorm.JoinBuilder, joinTable, curTable clause.Table) error {
				if v, ok := MaybeEmpty(p.Roles).Get(); ok {
	...
	return Do(func() Unit {
		...
		user := Must(
			gorm.G[model.Account](db).
				Where(gen.Account.Sub.Eq(Must(uuid.Parse(payload.ID)))).
...
func upsertUser(c fiber.Ctx) error {
	return Do(func() Unit {
		...
		acc := model.Account{
			...
			PhoneNumber:       MaybeEmpty(payload.PhoneNumber).ToPointer(),
		}

		MustPass(gorm.G[model.Account](db, clause.OnConflict{
		...
		MustPass(gorm.G[model.Admin](db, clause.OnConflict{
...
func getAllRoles(c fiber.Ctx) Result[[]rbac.Role] {
	return Do(func() []rbac.Role {
