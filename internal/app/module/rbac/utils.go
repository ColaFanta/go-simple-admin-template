...
func GetAllRoles(ef *casbin.Enforcer) opera.Result[[]Role] {
	return opera.Do(func() []Role {

		crolestrs := opera.Must(ef.GetAllRoles()) // Get g-v1

		cuserstrss := lo.Map(crolestrs, func(item string, _ int) []string {
			return opera.Try(ef.GetUsersForRole(item)).OrEmpty() // Get g-v0
		})
		...
		roles := lo.Map(cuserstrs, func(item string, _ int) Role {
			...
			policies := opera.Try(ef.GetImplicitPermissionsForUser(item)).OrEmpty()
